(() => {
  ...
  const titles = {dashboard:'ภาพรวม',registry:'ทะเบียนผ่าตัด',followup:'ติดตามผลหลังผ่าตัด',monthly:'สถิติรายเดือน',trend:'แนวโน้มผลลัพธ์',quality:'คุณภาพและการทบทวน',reports:'รายงานผลลัพธ์',users:'จัดการสมาชิก',targets:'เป้าหมาย KPI'};
  ...
  function renderRegistry(root){
    ...
    if(!list.length)empty(p,q?'ไม่พบข้อมูลตามคำค้น':'ยังไม่มีรายการ เริ่มบันทึกได้จาก “เพิ่มผู้ป่วยใหม่”');
      ...
      const row=el('article','record'),info=el('div');info.append(el('h2','record-title',`${c.patient_name} · ${c.hn}`),el('div','record-meta',`${c.surgery_date} · ${c.eye} · ${c.surgeon} · ${c.procedure}`),el('div','record-meta',`Implant (Lens) / Power: ${c.implant||'ไม่ระบุ'} · Type Lens: ${c.lens_type||'ไม่ระบุ'}`));
  ...
  function openCase(record=null){
    closeMenu();
    const lens=$('#case-form').elements.lens_type;lens.querySelectorAll('[data-legacy]').forEach(o=>o.remove());
    if(record?.lens_type&&!Clinical.lensTypes.includes(record.lens_type)){const option=el('option','',`ข้อมูลเดิม: ${record.lens_type}`);option.value=record.lens_type;option.dataset.legacy='true';lens.append(option);}
  ...
  function renderTargets(root) {
    ...
    state.targets.filter(t=>t.key!=='refractive').forEach(t=>{
  ...
  function targetFormula(source,formula) {
    ...
    const definition=Clinical.sourceDefinitions.find(d=>d[0]===source);
  ...
  function openTarget(target=null) {
    ...
    f.elements.source.replaceChildren(...[...Clinical.sourceDefinitions,['custom','กำหนดสูตรเอง']].map(([key,label])=>{const option=el('option','',label);option.value=key;return option;}));
